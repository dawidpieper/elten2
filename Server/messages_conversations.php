...
$limit=1000;
$q=mquery("select if(sender='{$_GET['name']}',receiver,sender) as user, sender, date, subject, if((if(sender='{$_GET['name']}',receiver,sender) in (select sender from messages where receiver='{$_GET['name']}' and `read` is null and deletedfromreceived=0)), 0, 1) as r, id from messages where id in (select max(id) from messages where ((sender='{$_GET['name']}' and deletedfromsent=0) or (receiver='{$_GET['name']}' and deletedfromreceived=0)) group by IF(sender > receiver, sender,receiver),         IF(sender > receiver, receiver,sender)) and ((sender='{$_GET['name']}' and deletedfromsent=0) OR (receiver='{$_GET['name']}' and deletedfromreceived=0)) group by user order by id desc limit 0,".((int)($limit+1)));
$cnt=0;
