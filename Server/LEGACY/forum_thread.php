...
$qposts=mquery("select p.id, p.author, p.post, p.date, p.polls, p.attachments, if(l.id is not null, 1, 0) from forum_posts p left join forum_posts_likes l on l.post=p.id and l.user='".mysql_real_escape_string($_GET['name'])."' where p.thread=". (int) $_GET['thread']."");
...
$posts[$r[0]]=[$r[0],$r[1],$r[2],$r[3],$r[4],$r[5], $signatures[$r[1]],$r[6]];
