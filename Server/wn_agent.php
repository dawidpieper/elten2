...
$qi=mquery("select id,sender,subject from messages where receiver='".mysql_real_escape_string($_GET['name'])."' and `read` is null and deletedfromreceived=0");
...
$qi=mquery("select id,lastpostdate,name from forum_threads where id in (select thread from followedthreads where owner='".mysql_real_escape_string($_GET['name'])."') and id in (select thread from forum_read r where owner='".mysql_real_escape_string($_GET['name'])."' and posts<(select count(*) from forum_posts where thread=r.thread));");
while($r=mysql_fetch_row($qi))
...
$qi=mquery("select postid,owner,name from blog_posts where posttype=0 and owner in (select author from followedblogs where owner='".mysql_real_escape_string($_GET['name'])."' and (owner,postid) not in (select author,postid from blog_read where owner='".mysql_real_escape_string($_GET['name'])."'))");
while($r=mysql_fetch_row($qi))
...
$qi=mquery("select postid,name from blog_posts where owner='".mysql_real_escape_string($_GET['name'])."' and postid in (select post from blog_read r where owner='".mysql_real_escape_string($_GET['name'])."' and author='".mysql_real_escape_string($_GET['name'])."' and posts<(select count(*) from blog_posts where owner='".mysql_real_escape_string($_GET['name'])."' and postid=r.post))");
while($r=mysql_fetch_row($qi))
array_push($ret['wn'],array('id'=>'blc_'.$r[0],'alert'=>substr($r[1],0,64),'sound'=>'notification_blogcomment'));
}
if(($_GET['client']==1 and $wnc[5]==0) or ($_GET['client']!=1 and $wnc[5]<2)) {
$qi=mquery("select id,name from forum_threads where id not in (select thread from forum_read where owner='".mysql_real_escape_string($_GET['name'])."') and forum in (select forum from followedforums where owner='".mysql_real_escape_string($_GET['name'])."')");
while($r=mysql_fetch_row($qi))
...
$qi=mquery("select id,lastpostdate,name from forum_threads where id in (select id from forum_threads where forum in (select forum from followedforums where owner='".mysql_real_escape_string($_GET['name'])."')) and id in (select thread from forum_read r where owner='".mysql_real_escape_string($_GET['name'])."' and posts<(select count(*) from forum_posts where thread=r.thread));");
while($r=mysql_fetch_row($qi))
...
$qi=mquery("select mentions.id, mentions.author, mentions.message, forum_threads.name from mentions, forum_threads where mentions.noticed is null and mentions.user='".$_GET['name']."' and forum_threads.id=mentions.thread");
while($r=mysql_fetch_row($qi))
