...
$q = mquery("SELECT `version`,`versiontype`,`beta` FROM `logins` WHERE `versiontype`!='WEB' AND `name`='".$_GET['searchname']."' ORDER BY `time` DESC");
$r=mysql_fetch_row($q);
$version = $r[0];
if($r[1]!="")
$version.=" ".$r[1]." ".$r[2];
...
$q = mquery("SELECT `id`,`time` FROM `logins` WHERE `name`='".$_GET['searchname']."' ORDER BY `time` ASC");
$registered=0;
...
if(mysql_num_rows($q)>0)
$registered=strtotime(mysql_fetch_row($q)[1]);
$q = mquery("SELECT `id`,`date` FROM `messages` WHERE `sender`='".$_GET['searchname']."' OR `receiver`='".$_GET['searchname']."' ORDER BY `id` ASC");
if(mysql_num_rows($q)>0)
$mregistered=strtotime(mysql_fetch_row($q)[1]);
if($mregistered<$registered and $mregistered != 0)
