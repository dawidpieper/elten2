...
require("header.php");
$q = mquery("SELECT `postid`, `author`, `post`, `date`, `moddate`, `privacy`, `comments` FROM `blog_posts` WHERE `owner`='" . mysql_real_escape_string($_GET['searchname']) . "' AND `postid`=" . (int)$_GET['postid']);
$re = 0;
$text = "";
$knownposts=0;
if($_GET['name']!="guest") {
...
if($_GET['details']==5 and $text==$knownposts."\r\n") $text.=$r[6]."\r\n";
