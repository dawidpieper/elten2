    ...
    def loop_update(checkControls = true, responseCalls = true)
      ...
      if $agent != nil and (av = $agent.avail) > 0
        ...
        for e in m
          ...
          if d["func"] == "notif"
          ...
          elsif d["func"] == "conference_waitingchannel"
          ...
          elsif d["func"] == "conference_diceroll"
            ...
            c = srvproc("calls", { "ac" => "absstatus", "call" => @@call.id })
            if c[0].to_i == 0 && c[1].to_i == 1
              @@missedcalls_window ||= MissedCallsWindow.new
              @@missedcalls_window.add_caller(@@call.caller)
            end
            @@call = nil
            $focus = true
