...
class Scene_Forum
  ...
  def groupsload(type, ll = nil)
    ...
    pinned = []
    pinned = LocalConfig["ForumGroupsPinned", []] if has_premiumpackage("courier")
    case type
    when 0
      ...
      grpselt[@grpheadindex + @sgroups.size + 9] = [nil] if !has_premiumpackage("courier")
  ...
  def context_forums(menu)
    if @frmsel.options.size > 0
      ...
      if @sforums[@frmsel.index].group.role == 2 || has_premiumpackage("courier")
    ...
    if @query != "" and @query.is_a?(String)
      ...
      if sr[0].to_i < 0
        ...
        for thread in @threads
          suc_th = false
          suc_se = false
          suc_se = true if @query.phrase_in.include?(:name) && thread.name.downcase.include?(@query.phrase.downcase)
          ...
          @results.push(thread.id) if suc_se && suc_th
        end
      end
...
class Scene_Forum_GroupSettings
  ...
  def load_general
    ...
    if has_premiumpackage("audiophile")
