  ...
  def main
    ...
    if @form.fields[1].checked == 1 or @form.fields[2].checked == 1
      ...
      if @form.fields[2].checked == 1
        ...
        createdirifneeded(@destdir + "\\eltendata\\soundthemes")
        d = Dir.entries(Dirs.soundthemes)
        for f in d
          next if File.extname(f).downcase != ".elsnd"
          loop_update
          copyfile(Dirs.soundthemes + "\\" + f, @destdir + "\\eltendata\\soundthemes\\" + f)
        end
        speech_wait
      end
    end
