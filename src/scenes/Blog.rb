...
class Scene_Blog
  ...
  def main
    ...
    @sel.disable_item(8) if !has_premiumpackage("courier")
    @sel.disable_item(7) if !has_premiumpackage("courier")
    @sel.focus
    loop do
      loop_update
      @sel.update
...
class Scene_Blog_Posts
  ...
  def context(menu)
    ...
    if @post.size > 0 && @sel.index < @post.size
      ...
      if has_premiumpackage("courier") || @post[@sel.index].followed
...
class Scene_Blog_Read
  ...
  def context(menu)
    ...
    if @iseltenblog
      ...
      if has_premiumpackage("courier") || @post.followed
        menu.option(opt, nil, "l") {
  ...
  def context(menu)
    ...
    menu.option(p_("Blog", "Create new blog"), nil, "n") {
      ...
      if b[0].to_i > 0
        ...
        can = false if b[1].to_i > 0 && !has_premiumpackage("scribe")
      end
      if can == true
        $bloglistindex = @sel.index
...
class Struct_Blog_Blog
  attr_accessor :id, :name, :description, :cnt_posts, :cnt_comments, :url, :lastpost, :followed, :lang
...
class Scene_Blog_PostEditor
  ...
  def main
    ...
    @form.hide(chk_schedule) if chk_schedule.checked == 0 && !has_premiumpackage("scribe")
...
class Scene_Blog_Domain
  ...
  def changer
    ...
    @lst_domaintype.disable_item(2) if !has_premiumpackage("scribe")
