...
class Scene_Messages
  ...
  def load_messages(user, subject, sp = nil, limit = @messages_limit || 50, complete = false)
    ...
    if sp == "flagged"
      ...
      msg = srvproc("messages_conversations", { "user" => user, "subj" => subject, "limit" => @messages_limit.to_s, "details" => 2 })
    end
    @messages_wn = 0
    if $agent_msg != nil
  ...
  def context_messages(menu)
    ...
    if @sel_messages.index < @messages.size and @messages[@sel_messages.index].receiver[0..0] != "["
      ...
      if has_premiumpackage("courier") || @messages[@sel_messages.index].protected == 1
        menu.option(s, nil, "c") {
...
class Scene_Messages_New
  ...
  def main
      ...
      if @attachments.size < 3
        menu.option(p_("Messages", "Attach a file"), nil, "n") {
          ...
          if loc != nil
            size = File.size(loc)
            atsize = 4 * 1024 ** 2
            atsize = 32 * 1024 ** 2 if has_premiumpackage("courier")
            if size > atsize
    ...
    @attachments = []
    @polls = []
    @form.hide(6) if !has_premiumpackage("courier")
    loop do
      loop_update
