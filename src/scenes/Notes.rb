...
class Scene_Notes_New
  def main
    @fields = [EditBox.new(p_("Notes", "note title"), "", "", true), EditBox.new(p_("Notes", "Note content"), EditBox::Flags::MultiLine, "", true), Button.new(p_("Notes", "Add")), Button.new(_("Cancel"))]
    @form = Form.new(@fields)
    btn = @form.fields[2]
    loop do
      loop_update
