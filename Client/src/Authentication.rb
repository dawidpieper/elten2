...
suc=true
phone=""
  password=input_text(p_("Authentication", "Type your password"),"PASSWORD|ACCEPTESCAPE") while password==""
  return main if password=="\004ESCAPE\004"
phone=input_text(p_("Authentication", " Type your phone number that will be used during verification. Remember to enter  the country code, for example, +48 for Poland"),"ACCEPTESCAPE") while (phone=="" or (phone[0..0]!="+" and phone[0..1]!="00") or phone.size<11 or (/[a-zA-Z,.\/;'\"\[\]!@\#\$%\^\&\*\(\)\_]/=~phone)!=nil) and phone!="\004ESCAPE\004"
...
return main if input_text(p_("Authentication", "Is this phone number correct? Press enter to continue or escape to cancel."),"ACCEPTESCAPE|READONLY",phone)=="\004ESCAPE\004"
if suc==true
...
if enb[0].to_i<0||enb[0].include?("-")
  ...
  password=""
  password=input_text(p_("Authentication", "Type your password"),"PASSWORD|ACCEPTESCAPE") while password==""
  if password!="\004ESCAPE\004" and confirm(p_("Authentication", "Are you sure you want to disable two-factor authentication?"))==1
