...
class Scene_Version
  def main
    ...
    fffmpeg=ChildProc.new("bin\\ffmpeg -version")
        f7zip=ChildProc.new("bin\\7z")
        frar=ChildProc.new("bin\\rar")
    loop_update while fruby.avail==0
    txt+=fruby.read+"\r\n\r\n"
    loop_update while fffmpeg.avail==0
    txt+=fffmpeg.read.split("\n")[0].delete("\r")+"\r\n"
    loop_update while f7zip.avail==0
    txt+=f7zip.read.split("\n")[1].delete("\r")+"\r\n"
    loop_update while frar.avail==0
    txt+=frar.read.split("\n")[1].delete("\r")+"\r\n"
