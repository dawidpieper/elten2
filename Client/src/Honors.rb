...
class Scene_Honors
  ...
  def main
    ...
    @sel=Select.new(selt,true,ind,header)
...
if $rang_moderator==1
   menu.option(p_("Honors", "Grant a badge")) {
                            user=input_text(p_("Honors", "Who should be granted this badge?"),"ACCEPTESCAPE")
                          if user!="\004ESCAPE\004"
                            if user_exist(user)==false
                              alert(p_("Honors", "The user cannot be found"))
                            else
   ...
   menu.option(p_("Honors", "Edit challenges")) {
                             ...
                             @sel.commandoptions[@sel.index]=makeselt(@honors[@sel.index])
                             loop_update
   }
  ...
  def editchallenges(honor)
    ...
    form=Form.new([Select.new(selt,true,0,p_("Honors", "Challenges"),true),Button.new(_("Save")),Button.new(_("Cancel"))])
    loop do
      loop_update
      form.update
    ...
    selt+=[p_("Honors", "Add challenge")]
    form.fields[0].commandoptions=selt
    play("edit_delete")
    form.fields[0].focus
  end
  ...
  if enter and form.index==0
    ...
    subform=Form.new([
    Edit.new(p_("Honors","Level"),"",levels[l]||"",true),
    Edit.new(p_("Honors","English level"),"",enlevels[l]||"",true),
    Button.new(_("Save")),
    Button.new(_("Cancel"))])
loop do
  loop_update
  ...
  if subform.fields[2].pressed?
    levels[l]=subform.fields[0].text_str
    enlevels[l]=subform.fields[1].text_str
    break
    end
  end
  selt=[]
...
main
  ...
  class Scene_Honors_New
    def main
      @form=Form.new([Edit.new(p_("Honors", "Badge name"),"","",true),Edit.new(p_("Honors", "Badge description"),"","",true),Edit.new(p_("Honors", "Badge name in English"),"","",true),Edit.new(p_("Honors", "Badge description in English"),"","",true),Button.new(p_("Honors", "Add")),Button.new(_("Cancel"))])
      loop do
        loop_update
        @form.update
