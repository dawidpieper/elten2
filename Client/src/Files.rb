...
if $key[0x11] and $key[0x44]
  ...
  if @tree.filetype==1
    ...
    if d<360000
        ...
        m=d/60%60
  s=d%60
  speech(sprintf("%02d:%02d:%02d",h,m,s)) if s!=nil
  end          
  end
  end
  end
...
m.option(p_("Files", "Add all audio files in this directory to playlist")) {
  ...
  m.option(p_("Files", "Rename")) {
      name=""
    while name==""
    name=input_text(p_("Files", "A new file name"),0,@tree.file, true)
    end
    if name != nil
  ...
  m.option(p_("Files", "New folder")) {
    name=""
while name==""
      name=input_text(p_("Files", "Enter a folder name"),0,"", true)
      end
    if name != nil
  ...
  name=""
    while name == ""
    name=input_text(p_("Files", "Enter a file name"),0,pr, true)
  end
  if name!=nil            
...
def textmenu(submenu=false)
  ...
  ck=0x25 if submenu
    ...
    case ind
    when 0
    form=Form.new([EditBox.new(@tree.file,EditBox::Flags::MultiLine,r),Button.new(_("Save")),Button.new(_("Cancel"))])
    loop do
      loop_update
      form.update
      break if escape or ((space or enter) and form.index == 2)
      if ((space or enter) and form.index == 1) or (enter and $key[0x11])
        writefile(@tree.path+@tree.file,form.fields[0].text.gsub("\004LINE\004","\r\n"))
        alert(_("Saved"))
        break
              end
            end
  ...
  ck=0x25 if submenu
    ...
    if ind<2 and ind>-1
      ...
      waiting
      convert_book(@tree.selected, Dirs.temp+"\\#{fid}.txt")
      return if !FileTest.exists?(Dirs.temp+"\\"+fid+".txt")
            text=readfile(Dirs.temp+"\\"+fid+".txt")
            waiting_end
      end
    case ind
    when 0
          File.delete(Dirs.temp+"\\"+fid+".txt")
          form=Form.new([EditBox.new(@tree.file,EditBox::Flags::MultiLine|EditBox::Flags::ReadOnly,text),Button.new(_("Exit"))])
    loop do
      loop_update
      form.update
      break if escape or ((space or enter) and form.index == 1)
            ...
            when 1
              speechtofile(Dirs.temp+"\\#{fid}.txt","",File.basename(@tree.file).gsub(File.extname(@tree.file),""))
                          end
            File.delete(Dirs.temp+"\\"+fid+".txt") if FileTest.exists?(Dirs.temp+"\\"+fid+".txt")
                          return (ind==-1)?false:true
            end
