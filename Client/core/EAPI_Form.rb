...
@repeat = @toundo
    ...
    if $key[0x11] == true and $key[89] == true
      ...
      if $key[0x11] == true and $key[84] == true
              gc = getcheck
              gc=text_str if gc.size<=2 or gc==nil
      if $key[0x10]==false
...
if @audiotext!=nil
  tp="Media"
  text_str="\004AUDIO\004#{@audiotext}\004AUDIO\004"+text_str
  end
  if @password == true
  text_str = ""
    end
...
def focus
   play("list_marker")
      ...
      txt += dict(@commandoptions[@index].to_s) if @commandoptions.size>0
      txt += dict("Pusta lista") if @commandoptions.size==0
      speech(txt)
    end
    def disable_item(id)
  @grayed[id] = true
...
speech(sp)
    ...
    if oldindex != self.index
pos=-1+((self.index).to_f/(options.size-1).to_f)*2
Thread.new do
play("list_focus")
end
  self.index = 0 if options.size == 1 or options[self.index] == nil
...
speech(sp)
    ...
    $focus = false
      ...
      def selector(options,header="",index=0,escapeindex=nil,type=0)
lsel=""
        if type == 0
        lsel = Select.new(options,true,index,header)
      else
        lsel = SelectLR.new(options,true,index,header)
        end
        loop do
          loop_update
          lsel.update
