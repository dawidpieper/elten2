...
class Conference
  ...
  def create_channel(params)
    ...
    resp = @voip.create_channel(name, public, framesize, bitrate, password, spatialization, channels, lang, width, height)
    if resp.is_a?(Hash)
      return resp["id"]
    else
      return nil
    ...
    loop {
      ...
      while queued.bytesize - index >= sf
        ...
        if !im
          ...
          frame = @speexdsp.process(frame) if @speexdsp != nil
          vorqueued += frame
